...
class MovableContentFalsePositiveTest : BasePlatformTestCase() {

  private val rule = MovableContentRule()

  fun test_movableContentInsideRememberFamily_shouldNotViolate() {
    val fn = configure(
      """
        annotation class Composable
        @Composable
        fun Foo(content: @Composable () -> Unit) {
          val m = rememberRetained { movableContentOf { content() } }
          m()
        }
      """.trimIndent(),
    )
  ...
  fun test_unrememberedMovableContent_shouldViolate() {
    val fn = configure(
      """
        annotation class Composable
        @Composable
