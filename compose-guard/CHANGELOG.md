...
- **MultipleContentEmitters** - Emitters inside `if`/`when` branches are now counted. The emission count previously looked only at direct call children of the function body, so content inside a conditional was invisible — `Text("a"); if (show) { Image(...) }` (two pieces of content on one path) and `if (loading) { Text(...); Image(...) }` (two emitters in a single branch) both went unreported. Conditionals now contribute their heaviest single branch (only one branch executes), so genuine multiple emissions are flagged while either/or alternatives — `if (c) Text() else Text()`, one arm per `when` entry — are correctly treated as a single emission and left alone.
- **ViewModelForwarding** - The rule now flags a ViewModel handed verbatim to another composable in any argument form. Previously it only fired for a *named* argument whose name was itself ViewModel-like (`ChildScreen(viewModel = viewModel)`), so it missed both the far more common positional call (`ChildScreen(viewModel)`) and forwarding under a differently-named child parameter (`ChildScreen(state = viewModel)`) — the VM is forwarded in all three. Detection now keys only on the VM being passed verbatim to a composable callee (PascalCase), while passing the ViewModel as an effect restart key (`LaunchedEffect(viewModel)` / `DisposableEffect(viewModel)`) is correctly exempted as a key rather than a forward.
...
### Tests
- Added a **SuppressComposeRuleFix behavior suite** for the suppress quick fix offered by nearly every rule, which previously had no coverage. It applies the fix and asserts the round-trip property — that `isSuppressed` then recognizes the result — plus no syntax errors, covering the new-annotation, merge-into-existing (preserving prior entries, no second `@Suppress`), already-suppressed (no duplicate), and no-modifier-list paths.
- Added a **line-marker provider test** asserting the gutter icon respects suppression (and appears for an unsuppressed violation) — the test behind the suppression fix above.
...
### Fixed (Quick Fixes)
- **Add modifier parameter** - The fix now places the new `modifier` parameter correctly when a *required* parameter is itself a collection of lambdas (e.g. `items: List<() -> Unit>`). The placement check matched a `->` anywhere in the type text, so the nested arrow made the required list look like a trailing content lambda and `modifier` was inserted in front of it, breaking the required-before-optional convention. The check now requires a top-level arrow, so `modifier` lands after required parameters and before the genuine content slot. (The helper was also renamed away from `isLambdaParameter` to avoid silently resolving to the Kotlin PSI's identically-named extension.)
...
### Fixed (Detection)
- **Inlay hints honour suppression** - The inline inlay hint (the icon + rule-id/“N issues” badge next to a composable's name) counted suppressed violations, so a rule suppressed via `@Suppress("RuleId")` or a `noinspection` comment still showed a hint, color and tooltip after its inline highlight was gone. The inlay provider now filters suppressed violations like every other surface. The collection logic was lifted to a testable `collectUnsuppressedViolations` helper.
