...
- **ViewModelForwarding** - Positional ViewModel forwarding — `ChildScreen(viewModel)`, the most common form — is now flagged. The rule previously only fired when the ViewModel was passed as a *named* argument whose name was itself ViewModel-like (`ChildScreen(viewModel = viewModel)`), so the far more common positional call slipped through entirely. Positional VM arguments handed to a composable callee (PascalCase) are now reported, while passing the ViewModel as an effect restart key (`LaunchedEffect(viewModel)` / `DisposableEffect(viewModel)`) is correctly exempted as a key rather than a forward. The conservative named-argument heuristic is unchanged.
