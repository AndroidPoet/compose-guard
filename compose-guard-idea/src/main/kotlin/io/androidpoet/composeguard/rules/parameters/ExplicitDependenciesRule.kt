...
public class ExplicitDependenciesRule : ComposableFunctionRule() {
  ...
  // Navigation-related call expressions where viewModel() is idiomatic
  ...
  override fun doAnalyze(function: KtNamedFunction, context: AnalysisContext): List<ComposeRuleViolation> {
    // Skip override functions - they're constrained by parent
    ...
    // Check if we're inside a navigation context (composable { }, NavHost, etc.)
    ...
    for (call in calls) {
      ...
      // Check for CompositionLocal.current access
  ...
  private fun isInNavigationContext(function: KtNamedFunction): Boolean {
    // Check if the function is defined inside a navigation lambda
