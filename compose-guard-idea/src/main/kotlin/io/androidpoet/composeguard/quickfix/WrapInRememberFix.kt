...
public class WrapInRememberFix : LocalQuickFix, HighPriorityAction {
  ...
  override fun applyFix(project: Project, descriptor: ProblemDescriptor) {
    ...
    // Find the expression to wrap - handle various PSI element types
    val expression: KtExpression = when (element) {
      is KtExpression -> element
      is KtProperty -> element.initializer
      else -> {
        // Try to find expression in parent
        element.parent as? KtExpression
          ?: (element.parent as? KtProperty)?.initializer
      }
    } ?: return

    // Get the file BEFORE replacing the expression (element becomes invalid after replace)
    val file = expression.containingKtFile

    // Analyze the expression to determine the best remember pattern
  ...
  private fun findUsedParameters(
    ...
    // Collect parameters that are actually used
