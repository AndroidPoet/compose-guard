...
public class ComposeGuardSettingsConfigurable : Configurable {
  ...
  private fun createCategoryPanel(
    ...
    return JPanel().apply {
      ...
      for (rule in rules) {
        val ruleCheckbox = JBCheckBox(rule.displayName, settings.isRuleEnabled(rule.id)).apply {
  ...
  override fun isModified(): Boolean {
    ...
    for ((ruleId, checkbox) in ruleCheckboxes) {
      if (checkbox.isSelected != settings.isRuleEnabled(ruleId)) return true
    }
  ...
  override fun reset() {
    ...
    for ((ruleId, checkbox) in ruleCheckboxes) {
      checkbox.isSelected = settings.isRuleEnabled(ruleId)
    }
